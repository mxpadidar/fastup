...
from fastup.core import commands, handlers, protocols, services
...
@router.post("/accounts/signup", status_code=202, response_model=resp_models.OtpResp)
async def issue_signup_otp(
    data: req_models.IssueSignupOtpReq,
    ...
    """Issue a signup OTP for phone number verification.

    Creates and sends a one-time password for user signup. The OTP is valid
    for a limited time and must be verified to complete registration.
    """
    cmd = commands.IssueSignupOtpCommand(
        phone=data.phone,
        ipaddr=ipaddr,
    )
    otp = await handlers.handle_issue_signup_otp(
        cmd=cmd, config=config, uow=uow, idgen=idgen, hmach_hasher=hmac_hasher
    )
