...
def test_getenv_raises_value_error_for_invalid_int_cast() -> None:
...
def test_getenv_raises_value_error_on_invalid_bool_cast() -> None:
    os.environ["TEST_BOOL_INVALID"] = "maybe"
    with pytest.raises(ValueError):
        envlib.getenv("TEST_BOOL_INVALID", True)


def test_getenv_raises_value_error_when_key_missing_and_no_fallback() -> None:
    with pytest.raises(ValueError):
        envlib.getenv("MISSING_KEY", None)


def test_getenv_raises_value_error_on_none_fallback() -> None:
    with pytest.raises(ValueError):
