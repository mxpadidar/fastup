...
from fastup.core.utils import get_utc_now
...
async def test_handle_issue_signup_otp_creates_and_persists_otp(
    ...
    async with uow:
        ...
        # Cleanup
        await uow.otps.delete(retrieved_otp)
        await uow.commit()
...
async def test_handle_issue_signup_otp_allows_multiple_otps_for_same_phone(
    ...
    # Cleanup
    async with uow:
        await uow.otps.delete(otp1)
        await uow.otps.delete(otp2)
        await uow.commit()
