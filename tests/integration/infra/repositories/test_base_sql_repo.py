...
async def test_repo_get_raises_exc_if_called_without_filters(repo: SQLRepository):
    """Verifies get() raises InternalExc when called without any filters."""
    with pytest.raises(exceptions.InternalExc):
        await repo.get()
...
async def test_repo_add_persists_entity(repo: SQLRepository, db_session: AsyncSession):
    ...
    assert persisted_user.email == new_user.email
...
async def test_repo_get_returns_correct_entity(user: User, repo: SQLRepository):
    """Verifies get() can retrieve an existing entity by other attributes."""
    retrieved_user = await repo.get(email=user.email)
    assert retrieved_user is not None
...
async def test_repo_refresh_updates_entity_state(
    ...
    new_email = "refreshed@example.com"
    await db_session.execute(
        sqlalchemy.update(users).where(users.c.id == user.id).values(email=new_email)
    )
    await db_session.commit()

    # The in-memory 'user' object still has the old email
    assert user.email != new_email

    await repo.refresh(user)

    # After refresh, the object should have the new email
    assert user.email == new_email
